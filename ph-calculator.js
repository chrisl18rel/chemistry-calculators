...
// ─────────────────────────────────────────────────────────
// HSB COLOR PICKER (ported exactly from solubility-curve.html)
// ─────────────────────────────────────────────────────────
(function() {
  ...
  function rgbToHsb(r,g,b){
    ...
    let h=0;
    if(d>0){ if(max===r) h=((g-b)/d+6)%6; else if(max===g) h=(b-r)/d+2; else h=(r-g)/d+4; h=Math.round(h*60); }
  ...
  // Inject required CSS for the popover once
  const CSS = `
  .ph-cp-popover{position:fixed;z-index:9999;background:#1e2030;border:1px solid #3a3a5a;border-radius:8px;
    box-shadow:0 10px 40px rgba(0,0,0,0.7);width:300px;padding:0;overflow:hidden;user-select:none;}
  .ph-cp-sb{display:block;width:100%;height:160px;cursor:crosshair;}
  .ph-cp-strips{display:flex;gap:8px;padding:8px 10px 4px;}
  .ph-cp-strip-col{flex:1;display:flex;flex-direction:column;gap:5px;}
  .ph-cp-hue{width:100%;height:14px;border-radius:7px;cursor:pointer;position:relative;
    background:linear-gradient(to right,#f00,#ff0,#0f0,#0ff,#00f,#f0f,#f00);}
  .ph-cp-thumb{position:absolute;top:50%;transform:translate(-50%,-50%);width:16px;height:16px;
    border-radius:50%;border:2px solid #fff;box-shadow:0 1px 4px rgba(0,0,0,0.6);pointer-events:none;}
  .ph-cp-preview-col{width:34px;display:flex;flex-direction:column;gap:5px;padding-top:2px;}
  .ph-cp-preview{width:34px;height:34px;border-radius:5px;border:1px solid rgba(255,255,255,0.15);}
  .ph-cp-inputs{display:flex;gap:5px;padding:4px 10px 8px;}
  .ph-cp-ig{flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;}
  .ph-cp-ig input[type=number]{width:100%;background:#0d1117;border:1px solid #3a3a5a;border-radius:4px;
    color:#e0e0e0;font-size:11px;text-align:center;padding:4px 2px;outline:none;}
  .ph-cp-ig input[type=number]:focus{border-color:#4a90e2;}
  .ph-cp-ig input[type=number]::-webkit-inner-spin-button{display:none;}
  .ph-cp-lbl{font-size:9px;color:#6a6a8a;text-transform:uppercase;letter-spacing:0.5px;}
  .ph-cp-hex{flex:1.4;}
  .ph-cp-hex input{width:100%;background:#0d1117;border:1px solid #3a3a5a;border-radius:4px;
    color:#e0e0e0;font-size:11px;font-family:'Courier New',monospace;text-align:center;padding:4px 2px;outline:none;}
  .ph-cp-hex input:focus{border-color:#4a90e2;}
  .ph-cp-modes{display:flex;justify-content:center;gap:4px;padding:0 10px 6px;}
  .ph-cp-mode{flex:1;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.6px;
    background:none;border:1px solid #3a3a5a;color:#6a6a8a;border-radius:3px;padding:3px 2px;cursor:pointer;}
  .ph-cp-mode.active{background:#3a3a5a;color:#e0e0e0;border-color:#5a5a7a;}
  .ph-swatch-btn{border-radius:5px;border:2px solid rgba(255,255,255,0.2);cursor:pointer;padding:0;
    position:relative;outline:none;transition:border-color 0.1s;}
  .ph-swatch-btn:hover{border-color:rgba(255,255,255,0.6);}`;

  if (!document.getElementById('ph-cp-style')) {
    const s = document.createElement('style'); s.id='ph-cp-style'; s.textContent=CSS;
  ...
  function openColorPicker(anchorEl, currentColor, onColorChange) {
    ...
    let {r:ir,g:ig,b:ib}=hexToRgb(currentColor||'#4a90e2');
    let {h:ih,s:is,v:iv}=rgbToHsb(ir,ig,ib);
    let cpH=ih,cpS=is,cpV=iv,cpMode='HSB';

    const pop=document.createElement('div'); pop.className='ph-cp-popover';
    pop.innerHTML=`
      <canvas class="ph-cp-sb" id="ph-cp-sb" width="300" height="160"></canvas>
      <div class="ph-cp-strips">
        <div class="ph-cp-strip-col">
          <div class="ph-cp-hue" id="ph-cp-hue"><div class="ph-cp-thumb" id="ph-cp-hue-thumb"></div></div>
        </div>
        <div class="ph-cp-preview-col"><div class="ph-cp-preview" id="ph-cp-preview"></div></div>
      </div>
      <div class="ph-cp-modes">
        <button class="ph-cp-mode active" data-mode="HSB">HSB</button>
        <button class="ph-cp-mode" data-mode="RGB">RGB</button>
      </div>
      <div class="ph-cp-inputs" id="ph-cp-inputs"></div>`;
    ...
    const sbCanvas=pop.querySelector('#ph-cp-sb'), sbCtx=sbCanvas.getContext('2d');
    const hueBar=pop.querySelector('#ph-cp-hue'), hueThumb=pop.querySelector('#ph-cp-hue-thumb');
    const preview=pop.querySelector('#ph-cp-preview'), inputsEl=pop.querySelector('#ph-cp-inputs');

    function drawSB(){
      const W=sbCanvas.width,H=sbCanvas.height;
      const gH=sbCtx.createLinearGradient(0,0,W,0); gH.addColorStop(0,'#fff'); gH.addColorStop(1,`hsl(${cpH},100%,50%)`);
      sbCtx.fillStyle=gH; sbCtx.fillRect(0,0,W,H);
      const gV=sbCtx.createLinearGradient(0,0,0,H); gV.addColorStop(0,'rgba(0,0,0,0)'); gV.addColorStop(1,'rgba(0,0,0,1)');
      sbCtx.fillStyle=gV; sbCtx.fillRect(0,0,W,H);
      const cx=(cpS/100)*W,cy=(1-cpV/100)*H;
      sbCtx.beginPath(); sbCtx.arc(cx,cy,7,0,Math.PI*2); sbCtx.strokeStyle='#fff'; sbCtx.lineWidth=2; sbCtx.stroke();
      sbCtx.beginPath(); sbCtx.arc(cx,cy,5,0,Math.PI*2); sbCtx.strokeStyle='rgba(0,0,0,0.4)'; sbCtx.lineWidth=1; sbCtx.stroke();
    }
    function updateHue(){ hueThumb.style.left=(cpH/360*100)+'%'; }
    function updatePreview(){
      const hex=hsbToHex(cpH,cpS,cpV);
      preview.style.background=hex; anchorEl.style.background=hex;
    ...
    function buildInputs(){
      ...
      if(cpMode==='HSB'){
        [['H',cpH,0,360],['S',cpS,0,100],['B',cpV,0,100]].forEach(([lbl,val,mn,mx])=>{
          const g=document.createElement('div'); g.className='ph-cp-ig';
          g.innerHTML=`<input type="number" min="${mn}" max="${mx}" value="${Math.round(val)}"><span class="ph-cp-lbl">${lbl}</span>`;
        ...
        [['R',r],['G',g],['B',b]].forEach(([lbl,val])=>{
          const gp=document.createElement('div'); gp.className='ph-cp-ig';
          gp.innerHTML=`<input type="number" min="0" max="255" value="${Math.round(val)}"><span class="ph-cp-lbl">${lbl}</span>`;
          gp.querySelector('input').addEventListener('change',()=>{
            const inputs=pop.querySelectorAll('.ph-cp-ig input');
            const rv=parseInt(inputs[0].value)||0,gv=parseInt(inputs[1].value)||0,bv=parseInt(inputs[2].value)||0;
            const hsb=rgbToHsb(Math.max(0,Math.min(255,rv)),Math.max(0,Math.min(255,gv)),Math.max(0,Math.min(255,bv)));
            cpH=hsb.h; cpS=hsb.s; cpV=hsb.v; refresh();
          });
          inputsEl.appendChild(gp);
        });
      }
      const hg=document.createElement('div'); hg.className='ph-cp-ig ph-cp-hex';
      const hex=hsbToHex(cpH,cpS,cpV).slice(1).toUpperCase();
      hg.innerHTML=`<input type="text" maxlength="6" value="${hex}"><span class="ph-cp-lbl">#</span>`;
      hg.querySelector('input').addEventListener('change',e=>{
        const v='#'+e.target.value.replace(/[^0-9a-fA-F]/g,'').padStart(6,'0').slice(0,6);
        const {r,g,b}=hexToRgb(v); const hsb=rgbToHsb(r,g,b);
        cpH=hsb.h; cpS=hsb.s; cpV=hsb.v; refresh();
      });
      inputsEl.appendChild(hg);
    }
    function refresh(){ drawSB(); updateHue(); updatePreview(); buildInputs(); }
    refresh();

    pop.querySelectorAll('.ph-cp-mode').forEach(btn=>{
      btn.addEventListener('click',()=>{
        pop.querySelectorAll('.ph-cp-mode').forEach(b=>b.classList.remove('active'));
    ...
    function sbPick(e){
      ...
      cpS=Math.round(Math.max(0,Math.min(1,(e.clientX-r.left)/r.width))*100);
      cpV=Math.round(Math.max(0,Math.min(1,1-(e.clientY-r.top)/r.height))*100);
      refresh();
    }
    sbCanvas.addEventListener('mousedown',e=>{ sbDrag=true; sbPick(e); e.preventDefault(); });
    document.addEventListener('mousemove',e=>{ if(sbDrag) sbPick(e); });
    document.addEventListener('mouseup',()=>{ sbDrag=false; });

    let hueDrag=false;
    function huePick(e){
      const r=hueBar.getBoundingClientRect();
      cpH=Math.min(359,Math.round(Math.max(0,(e.clientX-r.left)/r.width)*360));
      refresh();
    }
    hueBar.addEventListener('mousedown',e=>{ hueDrag=true; huePick(e); e.preventDefault(); });
    document.addEventListener('mousemove',e=>{ if(hueDrag) huePick(e); });
    document.addEventListener('mouseup',()=>{ hueDrag=false; });

    setTimeout(()=>{ document.addEventListener('mousedown',_outsideClick); },20);
  }

  function _outsideClick(e){ if(_colorPopover&&!_colorPopover.contains(e.target)) closeColorPicker(); }
  function closeColorPicker(){
    if(_colorPopover){ _colorPopover.remove(); _colorPopover=null; }
    document.removeEventListener('mousedown',_outsideClick);
  }

  // Helper: make a swatch button that opens the picker
  function makeSwatchBtn(id, defaultColor, size) {
    const sz = size || '28px';
    return `<button class="ph-swatch-btn" id="${id}"
      style="background:${defaultColor};width:100%;height:${sz};"
      onclick="PhCalculator._openPicker(this, '${id}')"></button>`;
  }
  ...
  function _buildUserPointsList() {
    const container = document.getElementById('ph-user-points-list');
    if (!container || !_tit) return;
    const { userPoints } = _tit;

    if (!userPoints.length) {
      container.innerHTML = `<div style="font-size:11px;color:var(--text-dim);padding:4px 0;">
        Click the graph to add points.</div>`;
      return;
    }
    ...
    container.innerHTML = userPoints.map((up, i) => `
      <div style="background:var(--bg);border:1px solid var(--border);border-radius:5px;
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;flex-wrap:wrap;">
          ...
          <label class="stoi-radio" style="font-size:11px;">
            <input type="checkbox" id="ph-pt-lines-${i}" ${up.showLines?'checked':''}
              onchange="PhCalculator.updateUserPoint(${i})" /> Show lines
          </label>
        </div>
