// color-picker.js
// Advanced HSB color picker — extracted from Solubility Curve Generator v1.8
...
function openColorPicker(anchorEl, currentColor, onColorChange) {
  ...
  let {r:ir, g:ig, b:ib} = hexToRgb(currentColor || '#e94560');
  ...
  if (top + 290 > window.innerHeight) top = rect.top - 296;
    ...
    if (cpMode === 'HSB') {
      ...
      [['R',r],['G',g],['B',b]].forEach(([lbl,val]) => {
        ...
        inp.addEventListener('change', () => {
          ...
          const rv = parseInt(inputs[0].value)||0;
          const gv = parseInt(inputs[1].value)||0;
          const bv = parseInt(inputs[2].value)||0;
